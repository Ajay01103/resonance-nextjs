...
function FileDropzone({
  ...
  return (
    <div
      {...getRootProps()}
      className={cn(
        "flex cursor-pointer flex-col items-center justify-center gap-4 overflow-hidden rounded-2xl border border-dashed bg-muted/20 px-6 py-10 transition-colors",
...
export function VoiceCreateForm({ scrollable, footer, onError }: VoiceCreateFormProps) {
  ...
  return (
    <form
      ...
      className={cn(
        "flex flex-col",
        scrollable ? "min-h-0 flex-1 overflow-hidden" : "gap-5",
      )}>
      <div
        className={cn(
          scrollable
            ? "no-scrollbar flex min-h-0 flex-1 flex-col gap-5 overflow-y-auto px-6 py-5"
            : "flex flex-col gap-5",
        )}>
